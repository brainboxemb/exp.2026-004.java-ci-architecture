...
The **release/canonical-artifact cache policy is now qualified** (issue #20). Normal PR/main canonical execution may use project-selected `none | local | shared` when all material inputs participate in validity. Exact-tag release remains an independent trust boundary: fresh/empty module output, no Maven build-output cache read/write, ordinary Maven lifecycle, full release evidence, and publication that reuses the prepared canonical output without rebuilding. Existing event-timing `v0.2.1` release run `35211641563` already proves this operational shape, so no redundant CI-17 was added. See [`docs/11-release-cache-policy.md`](docs/11-release-cache-policy.md).

The initial Java CI PoP/qualification set is now sufficient for a cross-project adoption decision. No production rollout is activated automatically.
